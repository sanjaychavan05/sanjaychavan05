 ...
 *
 * Env vars:
 *   GH_USERNAME  - GitHub login to fetch contributions for (default: sanjaychavan05)
 *   GH_TOKEN     - token with access to the GraphQL API (optional).
...
const USERNAME = process.env.GH_USERNAME || "sanjaychavan05";
...
async function fetchWeeksGraphQL() {
...
async function fetchWeeksPublicHTML() {
  console.log(`Fetching public contribution page for ${USERNAME}...`);
  const res = await fetch(`https://github.com/users/${USERNAME}/contributions`);
  if (!res.ok) {
    throw new Error(`Failed to fetch public contributions: HTTP ${res.status}`);
  }
  ...
  while ((match = tooltipRegex.exec(html)) !== null) {
    ...
    const count = countMatch ? (countMatch[1].toLowerCase() === 'no' ? 0 : parseInt(countMatch[1])) : 0;
    tooltips[id] = count;
  }
  ...
  while ((match = dayRegex.exec(html)) !== null) {
    const [_, date, id, level] = match;
    const count = tooltips[id] !== undefined ? tooltips[id] : (parseInt(level) > 0 ? parseInt(level) : 0);
    days.push({ date, contributionCount: count, color: levelColors[level] || "#161b22" });
  }
...
async function fetchWeeks() {
  if (TOKEN) {
    try {
      ...
      return await fetchWeeksGraphQL();
    } catch (err) {
      console.warn(`GraphQL API fetch failed: ${err.message}. Falling back to public HTML fetcher.`);
    }
  }
  return await fetchWeeksPublicHTML();
}
...
async function main() {
  ...
  console.log(`Successfully generated heatmap SVG at: ${outPath}`);
}

main().catch((err) => {
  console.error("Error generating jet heatmap:", err);
  process.exit(1);
});
